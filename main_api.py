...
FAKE_USERS_DB = {
    "admin": {
        "username": "admin",
        "hashed_password": "$2b$12$m38XZWACbkP/tqVKuuNLUenFMGqaCUdvg37NgkNJDQsXLIPWA1QP.",  # Хэш для пароля "admin"
    }
}
...
def read_products(
    ...
    if stock_status == StockStatusFilter.LOW_STOCK:
        query = query.where((Product.stock_quantity <= Product.min_stock_level) & (Product.stock_quantity > 0))
    elif stock_status == StockStatusFilter.OUT_OF_STOCK:
        query = query.where(Product.stock_quantity == 0)

    count_query = select(func.count()).select_from(query.subquery())
    total_count = session.exec(count_query).one()
    paginated_query = query.offset(offset).limit(size).order_by(Product.is_favorite.desc(), Product.name)
...
def delete_worker(current_user: Annotated[dict, Depends(get_current_user)], worker_id: int, session: Session = Depends(get_session)):
    ...
    if not db_worker:
        raise HTTPException(status_code=404, detail="Работник не найден")
    has_movements = session.exec(select(StockMovement).where(StockMovement.worker_id == worker_id)).first()
    if has_movements:
        raise HTTPException(status_code=400, detail="Нельзя удалить работника, так как за ним числятся движения товаров.")
...
def issue_item_to_worker(current_user: Annotated[dict, Depends(get_current_user)], request: IssueItemRequest, session: Session = Depends(get_session)):
    product, worker = session.get(Product, request.product_id), session.get(Worker, request.worker_id)
    if not product: raise HTTPException(status_code=404, detail="Товар не найден")
    if not worker: raise HTTPException(status_code=404, detail="Работник не найден")
    if request.quantity <= 0: raise HTTPException(status_code=400, detail="Количество должно быть > 0")
    if product.stock_quantity < request.quantity: raise HTTPException(status_code=400, detail=f"Недостаточно товара. В наличии: {product.stock_quantity}")
...
def get_worker_stock(current_user: Annotated[dict, Depends(get_current_user)], worker_id: int, session: Session = Depends(get_session)):
    ...
    if not worker:
        raise HTTPException(status_code=404, detail="Работник с таким ID не найден")
    results = session.query(
        Product.id, Product.name, Product.unit, func.sum(StockMovement.quantity)
...
def write_off_item_from_worker(current_user: Annotated[dict, Depends(get_current_user)], request: WriteOffItemRequest, session: Session = Depends(get_session)):
    ...
    current_on_hand_sum = session.exec(on_hand_balance_query).one_or_none() or 0
    quantity_on_hand = -current_on_hand_sum

    if quantity_on_hand < request.quantity:
        raise HTTPException(status_code=400, detail=f"У работника на руках только {quantity_on_hand} шт. Нельзя списать {request.quantity} шт.")
...
def get_history(current_user: Annotated[dict, Depends(get_current_user)], session: Session = Depends(get_session)):
    history_records = session.exec(select(StockMovement).order_by(StockMovement.id.desc())).all()
    response = []
...
def cancel_movement(current_user: Annotated[dict, Depends(get_current_user)], movement_id: int, session: Session = Depends(get_session)):
    ...
    if not original_movement: raise HTTPException(status_code=404, detail="Движение не найдено.")
    if "Отмена" in original_movement.type: raise HTTPException(status_code=400, detail="Нельзя отменить операцию отмены.")
    product = session.get(Product, original_movement.product_id)
    if not product: raise HTTPException(status_code=404, detail="Связанный товар был удален.")
...
async def universal_import_v9(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Не удалось прочитать файл Excel. Ошибка: {e}")
    ...
    for i, row in df.iterrows():
        ...
        header_map_raw = {str(v).strip().upper(): col_idx for col_idx, v in enumerate(row.values)}
        if petrovich_headers.issubset(row_values):
            start_row = i
            header_map = {'sku': header_map_raw.get('КОД'), 'name': header_map_raw.get('ТОВАР'), 'qty': header_map_raw.get('КОЛИЧЕСТВО'), 'price': header_map_raw.get('ЦЕНА')}
            break
        elif my_sklad_headers.issubset(row_values):
            start_row = i
            header_map = {'internal_sku': header_map_raw.get('INTERNAL_SKU'), 'name': header_map_raw.get('NAME'), 'qty': header_map_raw.get('STOCK_QUANTITY'), 'sku': header_map_raw.get('SUPPLIER_SKU')}
            break
    if start_row == -1:
        raise HTTPException(status_code=400, detail="Не найдены заголовки ('КОД', 'ТОВАР'...) или ('INTERNAL_SKU', 'NAME'...)")
    ...
    if mode == ImportMode.TO_STOCK:
        ...
        for i, row in data_df.iterrows():
            try:
                ...
                if not name_val or qty_val is None: continue
                qty = float(qty_val)
                ...
                internal_sku = str(internal_sku_val).strip() if internal_sku_val else None

                product: Optional[Product] = None
                if sku: product = session.exec(select(Product).where(Product.supplier_sku == sku)).first()
                if not product and internal_sku: product = session.exec(select(Product).where(Product.internal_sku == internal_sku)).first()

                if product:
                    product.stock_quantity = qty if is_initial_load else product.stock_quantity + qty
                    product.purchase_price = price
                    session.add(product)
                    movement = StockMovement(product_id=product.id, quantity=qty, type=MovementTypeEnum.INCOME, stock_after=product.stock_quantity)
                    session.add(movement)
                    ...
                    new_product = Product(name=str(name_val), supplier_sku=sku, internal_sku=final_internal_sku, stock_quantity=qty, purchase_price=price, retail_price=price * 1.2)
                    session.add(new_product)
                    session.flush()
                    movement = StockMovement(product_id=new_product.id, quantity=qty, type=MovementTypeEnum.INCOME, stock_after=new_product.stock_quantity)
                    session.add(movement)
                    report["created"].append(f"{name_val}")
                else:
                    report["skipped"].append(f"{name_val} (артикул {sku or internal_sku})")
            except Exception as e:
        ...
        sku_col, qty_col, price_col = header_map.get('sku'), header_map.get('qty'), header_map.get('price')
        for i, row in data_df.iterrows():
            try:
                sku_val, qty_val, price_val = row.get(sku_col), row.get(qty_col), row.get(price_col) if price_col is not None else 0.0
                sku = str(sku_val).strip() if sku_val else None
                if not sku or qty_val is None: continue
                qty, price = float(qty_val), float(price_val or 0.0)
                product = session.exec(select(Product).where(Product.supplier_sku == sku)).first()
                if product:
                    items_to_create.append({"product_id": product.id, "quantity": qty, "price_from_file": price})
                else:
                    not_found_skus.append(sku)
            except (ValueError, TypeError, KeyError): continue
        if not_found_skus: raise HTTPException(status_code=404, detail=f"Товары с артикулами не найдены: {', '.join(not_found_skus)}")
        if not items_to_create: raise HTTPException(status_code=400, detail="Не найдено корректных товаров для сметы.")

        order_number = "б/н"
        try:
            order_cell_df = df[df.apply(lambda r: r.astype(str).str.contains('Заказ №', na=False).any(), axis=1)]
            if not order_cell_df.empty:
                order_cell = order_cell_df.values[0]
                order_number = str(next(s for s in order_cell if 'Заказ №' in str(s))).replace("Заказ №", "").strip()
        except Exception: pass

        new_estimate = Estimate(estimate_number=f"Импорт-{order_number}", client_name="Импорт из файла", location="Петрович")
        session.add(new_estimate)
        session.flush()
        for item in items_to_create:
            est_item = EstimateItem(product_id=item["product_id"], quantity=item["quantity"], unit_price=item["price_from_file"], estimate_id=new_estimate.id)
...
def create_estimate(current_user: Annotated[dict, Depends(get_current_user)], request: EstimateCreate, session: Session = Depends(get_session)):
    new_estimate = Estimate(estimate_number=request.estimate_number, client_name=request.client_name, location=request.location)
    for item_data in request.items:
        product = session.get(Product, item_data.product_id)
        if not product: raise HTTPException(status_code=404, detail=f"Товар с ID {item_data.product_id} не найден")
...
def read_estimates(
    ...
    if search:
        ...
        query = query.where(or_(Estimate.estimate_number.ilike(search_term), Estimate.client_name.ilike(search_term), Estimate.location.ilike(search_term)))
    count_query = select(func.count()).select_from(query.subquery())
    total_count = session.exec(count_query).one()
    paginated_query = query.offset(offset).limit(size).order_by(Estimate.id.desc())
    items = session.exec(paginated_query).all()
...
def read_estimate(current_user: Annotated[dict, Depends(get_current_user)], estimate_id: int, session: Session = Depends(get_session)):
    ...
    for item in estimate.items:
        ...
        response_items.append(EstimateItemResponse(id=item.id, quantity=item.quantity, unit_price=item.unit_price, product_id=item.product_id, product_name=product.name))
...
def update_estimate(current_user: Annotated[dict, Depends(get_current_user)], estimate_id: int, request: EstimateUpdate, session: Session = Depends(get_session)):
    ...
    if not db_estimate: raise HTTPException(status_code=404, detail="Смета не найдена")
    if db_estimate.status == EstimateStatusEnum.COMPLETED: raise HTTPException(status_code=400, detail="Нельзя редактировать завершенную смету.")
    if request.items is not None and db_estimate.status == EstimateStatusEnum.IN_PROGRESS: raise HTTPException(status_code=400, detail="Состав отгруженной сметы можно менять только через 'довыдачу'.")

    update_data = request.model_dump(exclude_unset=True)
    for key, value in update_data.items():
        if key != "items": setattr(db_estimate, key, value)

    if request.items is not None:
        items_to_delete = session.exec(select(EstimateItem).where(EstimateItem.estimate_id == estimate_id)).all()
        for item in items_to_delete: session.delete(item)
        for item_data in request.items:
            product = session.get(Product, item_data.product_id)
            if not product: raise HTTPException(status_code=404, detail=f"Товар ID {item_data.product_id} не найден.")
            unit_price = item_data.unit_price if item_data.unit_price is not None else product.retail_price
            new_item = EstimateItem(estimate_id=estimate_id, product_id=product.id, quantity=item_data.quantity, unit_price=unit_price)
...
def delete_estimate(current_user: Annotated[dict, Depends(get_current_user)], estimate_id: int, session: Session = Depends(get_session)):
    ...
    if db_estimate.status not in [EstimateStatusEnum.DRAFT, EstimateStatusEnum.APPROVED, EstimateStatusEnum.CANCELLED]:
        raise HTTPException(status_code=400, detail=f"Нельзя удалить смету в статусе '{db_estimate.status.value}'.")
    items_to_delete = session.exec(select(EstimateItem).where(EstimateItem.estimate_id == estimate_id)).all()
    for item in items_to_delete: session.delete(item)
...
def ship_estimate(current_user: Annotated[dict, Depends(get_current_user)], estimate_id: int, worker_id: int = Query(...), session: Session = Depends(get_session)):
    estimate, worker = session.get(Estimate, estimate_id), session.get(Worker, worker_id)
    if not estimate: raise HTTPException(status_code=404, detail="Смета не найдена")
    if not worker: raise HTTPException(status_code=404, detail="Работник не найден")
    if estimate.status not in [EstimateStatusEnum.DRAFT, EstimateStatusEnum.APPROVED]:
        raise HTTPException(status_code=400, detail=f"Нельзя отгрузить смету в статусе '{estimate.status.value}'")

    for item in estimate.items:
        product = session.get(Product, item.product_id)
        if product.stock_quantity < item.quantity:
            raise HTTPException(status_code=400, detail=f"Недостаточно товара '{product.name}'. В наличии: {product.stock_quantity}, требуется: {item.quantity}")
        product.stock_quantity -= item.quantity
        movement = StockMovement(product_id=item.product_id, worker_id=worker.id, quantity=-item.quantity, type=MovementTypeEnum.ISSUE_TO_WORKER, stock_after=product.stock_quantity)
        session.add(product)
        session.add(movement)
...
def issue_additional_items(current_user: Annotated[dict, Depends(get_current_user)], estimate_id: int, request: AddItemsRequest, session: Session = Depends(get_session)):
    estimate = session.get(Estimate, estimate_id)
    if not estimate: raise HTTPException(status_code=404, detail="Смета не найдена")
    if not estimate.worker_id: raise HTTPException(status_code=400, detail="Смета еще не отгружена, нельзя сделать довыдачу.")
    worker = session.get(Worker, estimate.worker_id)

    for item_data in request.items:
        product = session.get(Product, item_data.product_id)
        if not product: raise HTTPException(status_code=404, detail=f"Товар ID {item_data.product_id} не найден.")
        if product.stock_quantity < item_data.quantity: raise HTTPException(status_code=400, detail=f"Недостаточно товара '{product.name}'.")

        product.stock_quantity -= item_data.quantity
        new_item = EstimateItem(estimate_id=estimate_id, product_id=product.id, quantity=item_data.quantity, unit_price=item_data.unit_price)
        movement = StockMovement(product_id=product.id, worker_id=worker.id, quantity=-item_data.quantity, type=MovementTypeEnum.ISSUE_TO_WORKER, stock_after=product.stock_quantity)
        session.add(product)
        session.add(new_item)
        session.add(movement)
...
def complete_estimate(current_user: Annotated[dict, Depends(get_current_user)], estimate_id: int, session: Session = Depends(get_session)):
    estimate = session.get(Estimate, estimate_id)
    if not estimate: raise HTTPException(status_code=404, detail="Смета не найдена")
    if estimate.status != EstimateStatusEnum.IN_PROGRESS: raise HTTPException(status_code=400, detail="Завершить можно только смету в статусе 'В работе'.")
    if not estimate.worker_id: raise HTTPException(status_code=400, detail="К смете не привязан работник.")

    estimate_items = session.exec(select(EstimateItem).where(EstimateItem.estimate_id == estimate_id)).all()
    for item in estimate_items:
        balance_query = select(func.sum(StockMovement.quantity)).where(StockMovement.worker_id == estimate.worker_id, StockMovement.product_id == item.product_id)
...
def generate_commercial_proposal_docx(current_user: Annotated[dict, Depends(get_current_user)], estimate_id: int, session: Session = Depends(get_session)):
    ...
    if not os.path.exists(template_path): raise HTTPException(status_code=500, detail="Шаблон КП не найден")
...
def write_off_pipes_and_complete_contract(current_user: Annotated[dict, Depends(get_current_user)], contract_id: int, session: Session = Depends(get_session)):
    ...
    if not contract.pipe_steel_used and not contract.pipe_plastic_used:
        raise HTTPException(status_code=400, detail="Не указано количество использованных труб.")

    messages = []
    pipe_skus_map = {"PIPE-STEEL-DRILL": contract.pipe_steel_used, "PIPE-PLASTIC-DRILL": contract.pipe_plastic_used}
    for sku_anchor, qty in pipe_skus_map.items():
        if qty and qty > 0:
            product_query = select(Product).where(or_(func.trim(Product.internal_sku) == sku_anchor.strip(), func.trim(Product.supplier_sku) == sku_anchor.strip()))
            product = session.exec(product_query).first()
            if not product: raise HTTPException(status_code=404, detail=f"Товар с 'якорным' артикулом '{sku_anchor}' не найден.")
            if product.stock_quantity < qty: raise HTTPException(status_code=400, detail=f"Недостаточно '{product.name}'.")
            product.stock_quantity -= qty
            movement = StockMovement(product_id=product.id, quantity=-qty, type=MovementTypeEnum.WRITE_OFF_CONTRACT, stock_after=product.stock_quantity)
            session.add(product)
            session.add(movement)
            messages.append(f"Списано '{product.name}': {qty} м.")

    if not messages: raise HTTPException(status_code=400, detail="Не указано количество труб > 0.")
...
def generate_contract_docx(current_user: Annotated[dict, Depends(get_current_user)], contract_id: int, session: Session = Depends(get_session)):
    ...
    if not os.path.exists(template_path):
        raise HTTPException(status_code=500, detail=f"Шаблон '{template_name}' не найден в папке 'templates'")
...
def get_profit_report(
    ...
    query = select(Estimate).where(Estimate.status.in_([EstimateStatusEnum.COMPLETED, EstimateStatusEnum.IN_PROGRESS]))
    if estimate_id:
        query = query.where(Estimate.id == estimate_id)
    elif start_date and end_date:
        end_date_inclusive = end_date + timedelta(days=1)
        query = query.where(Estimate.created_at >= start_date, Estimate.created_at < end_date_inclusive)
    else:
    ...
    for estimate in estimates:
        ...
        if not estimate.worker_id: continue
        for item in estimate.items:
    ...
    average_margin = (grand_total_profit / grand_total_retail * 100) if grand_total_retail > 0 else 0
