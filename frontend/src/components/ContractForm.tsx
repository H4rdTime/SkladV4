            ...
            {isEditing ? (
                ...
                <form onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
                    <div className="bg-white p-6 rounded-lg shadow-md">
                        ...
                        <fieldset className="border p-4 rounded-md mb-6"><legend className="px-2 font-semibold">Основные данные</legend><div className="grid grid-cols-1 md:grid-cols-3 gap-4"><input name="contract_number" value={formData.contract_number || ''} onChange={handleChange} placeholder="Номер договора" required className="border p-2 rounded-md"/><input name="client_name" value={formData.client_name || ''} onChange={handleChange} placeholder="Имя клиента" required className="border p-2 rounded-md"/><input name="location" value={formData.location || ''} onChange={handleChange} placeholder="Адрес объекта" required className="border p-2 rounded-md"/></div></fieldset>
                        <fieldset className="border p-4 rounded-md mb-6"><legend className="px-2 font-semibold">Паспортные данные</legend><div className="grid grid-cols-1 md:grid-cols-3 gap-4"><input name="passport_series_number" value={formData.passport_series_number || ''} onChange={handleChange} placeholder="Серия и номер" className="border p-2 rounded-md"/><input name="passport_dep_code" value={formData.passport_dep_code || ''} onChange={handleChange} placeholder="Код подразделения" className="border p-2 rounded-md"/><input name="passport_issue_date" value={formData.passport_issue_date || ''} onChange={handleChange} placeholder="Дата выдачи" className="border p-2 rounded-md"/><div className="md:col-span-3"><input name="passport_issued_by" value={formData.passport_issued_by || ''} onChange={handleChange} placeholder="Кем выдан" className="border p-2 rounded-md w-full"/></div><div className="md:col-span-3"><input name="passport_address" value={formData.passport_address || ''} onChange={handleChange} placeholder="Адрес регистрации" className="border p-2 rounded-md w-full"/></div></div></fieldset>
                        <fieldset className="border p-4 rounded-md mb-6"><legend className="px-2 font-semibold">Параметры бурения</legend><div className="grid grid-cols-2 md:grid-cols-4 gap-4"><input type="number" step="0.1" name="estimated_depth" value={formData.estimated_depth ?? ''} onChange={handleChange} placeholder="План. глубина, м" className="border p-2 rounded-md"/><input type="number" step="0.01" name="price_per_meter_soil" value={formData.price_per_meter_soil ?? ''} onChange={handleChange} placeholder="Цена (до скалы)" className="border p-2 rounded-md"/><input type="number" step="0.01" name="price_per_meter_rock" value={formData.price_per_meter_rock ?? ''} onChange={handleChange} placeholder="Цена (по скале)" className="border p-2 rounded-md"/><div></div><input type="number" step="0.1" name="actual_depth_soil" value={formData.actual_depth_soil ?? ''} onChange={handleChange} placeholder="Факт (до скалы), м" className="border p-2 rounded-md"/><input type="number" step="0.1" name="actual_depth_rock" value={formData.actual_depth_rock ?? ''} onChange={handleChange} placeholder="Факт (по скале), м" className="border p-2 rounded-md"/><input type="number" step="0.1" name="pipe_steel_used" value={formData.pipe_steel_used ?? ''} onChange={handleChange} placeholder="Исп. сталь, м" className="border p-2 rounded-md"/><input type="number" step="0.1" name="pipe_plastic_used" value={formData.pipe_plastic_used ?? ''} onChange={handleChange} placeholder="Исп. пластик, м" className="border p-2 rounded-md"/></div></fieldset>
                ...
                <div className="bg-white p-6 rounded-lg shadow-md">
                    <div className="flex justify-between items-start mb-6 pb-4 border-b">
                        <div>
                            ...
                            <p className="text-gray-500">Статус: <span className="font-semibold text-green-600">{formData.status}</span></p>
                        </div>
                    ...
                    <div className="space-y-6">
                        <div><h3 className="font-semibold text-lg mb-2">Основные данные</h3><div className="grid grid-cols-1 md:grid-cols-3 gap-4"><ViewField label="Клиент" value={formData.client_name} /><ViewField label="Объект" value={formData.location} /><ViewField label="Дата договора" value={new Date(formData.contract_date || '').toLocaleDateString('ru-RU')} /></div></div>
                        <div><h3 className="font-semibold text-lg mb-2">Фактические результаты</h3><div className="grid grid-cols-2 md:grid-cols-4 gap-4"><ViewField label="Исп. сталь, м" value={formData.pipe_steel_used} /><ViewField label="Исп. пластик, м" value={formData.pipe_plastic_used} /><ViewField label="Факт (до скалы), м" value={formData.actual_depth_soil} /><ViewField label="Факт (по скале), м" value={formData.actual_depth_rock} /></div></div>
                        <div><h3 className="font-semibold text-lg mb-2">Паспортные данные</h3><div className="grid grid-cols-2 md:grid-cols-3 gap-4"><ViewField label="Серия и номер" value={formData.passport_series_number} /><ViewField label="Код" value={formData.passport_dep_code} /><ViewField label="Дата выдачи" value={formData.passport_issue_date} /><div className="md:col-span-3"><ViewField label="Кем выдан" value={formData.passport_issued_by} /></div><div className="md:col-span-3"><ViewField label="Адрес регистрации" value={formData.passport_address} /></div></div></div>
                    </div>
                </div>
            )}
        </main>
